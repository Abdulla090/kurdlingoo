...
const Admin = () => {
    ...
    const [editMode, setEditMode] = useState(null); // 'unit', 'lesson', 'exercise'
    ...
    const handleEditGuidebook = () => {
        setEditingItem(selectedUnit.guidebook || { content: '', keyPhrases: [] });
        setEditMode('guidebook');
    };

    const handleSaveGuidebook = (updatedGuidebook) => {
        const updatedUnits = units.map(u => {
            if (u.id === selectedUnit.id) {
